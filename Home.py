...
# Handle display based on radio button selection
if shot_type_radio == "Shots Taken":
    ...
    # Shots Team A(8)
    team_shot_counts = target_shots['teamName'].value_counts()
    team_display = [f"{team} ({count})" for team, count in team_shot_counts.items()]
    ...
    # First version | YT
    # player = st.selectbox('Select a player', target_shots[target_shots['teamName'] == team]['playerName'].sort_values().unique(), index=None)

    # Filter players by selected team | Just the name
    # player_options = target_shots[target_shots['teamName'] == team]['playerName'].sort_values().unique() if team else []
    # player = st.selectbox('Select a player', player_options, index=None, placeholder='Select a player')

    # Filter players by selected team | Colidio (10)
    if team:
        players_df = target_shots[target_shots['teamName'] == team]
        player_shot_counts = players_df['playerName'].value_counts()
        player_display = [f"{player} ({count})" for player, count in player_shot_counts.items()]
        display_to_player = {f"{player} ({count})": player for player, count in player_shot_counts.items()}
    else:
    ...
    def filter_data(target_shots, team, player):
        if team:
            target_shots = target_shots[target_shots['teamName'] == team]
        if player:
            target_shots = target_shots[target_shots['playerName'] == player]

        return target_shots

    filtered_df = filter_data(target_shots, team, player)
    ...
    def plot_shots(target_shots, ax, pitch):
        for x in target_shots.to_dict(orient='records'):
            pitch.scatter(
                x=x['x'],
                y=x['y'],
                s=800 * x['expectedGoals'],
                color = purple if x['eventType'] == 'Goal' else back_color,
                edgecolors=clean_white,
                linewidth=.8,
                alpha=1 if x['eventType'] == 'Goal' else .5, # Full opacity (1) for Goals, semi-transparent (0.5) for other shots
                zorder=2 if x['eventType'] == 'Goal' else 1, # Goals are drawn on top (2) of non-goals (1)
